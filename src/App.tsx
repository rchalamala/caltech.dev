	...
	for (let i = 0; i < 5; i++) {
		for (const intervalA of timesA[i]) {
			for (const intervalB of timesB[i]) {
				if ((intervalA!.start >= intervalB!.start && intervalA!.start < intervalB!.end) || (intervalB!.start >= intervalA!.start && intervalB!.start < intervalA!.end)) {
					return true;
				}
			}
		}
...
function App() {
	...
	const [workspaces, setWorkspaces] = useState<Workspace[]>(localWorkspaces ? JSON.parse(localWorkspaces) : [emptyWorkspace(), emptyWorkspace(), emptyWorkspace(), emptyWorkspace(), emptyWorkspace()]);
	const localWorkspaceIdx = localStorage.getItem("workspaceIdx");
	const [workspaceIdx, setWorkspaceIdx] = useState<number>(localWorkspaceIdx ? JSON.parse(localWorkspaceIdx) : 0);
	...
	const addCourse = (newCourse: CourseStorage) => {
		const result = courses.find((course) => course.courseData.id === newCourse.courseData.id);
		let newCourses: CourseStorage[] = [];
		if (result) {
			newCourses = courses.map((course) => (course.courseData.id === newCourse.courseData.id ? setField(newCourse, "locked", true) : course));
		} else {
	...
	const updateAvailableTimes = (dayIdx: number, isStart: boolean, day: Date) => {
		const newAvailableTimes = setArrayIdx(availableTimes, dayIdx, [isStart ? day : availableTimes[dayIdx][0], isStart ? availableTimes[dayIdx][1] : day]);
		let newCourses = courses;
		...
		if (newArrangements.length === 0) {
			...
			const isWidening = (isStart && day < availableTimes[dayIdx][0]) || (!isStart && day > availableTimes[dayIdx][1]);
		...
		>
			<div className="scroll-smooth antialiased selection:bg-orange-400 selection:text-black">
				<div className="sticky-help">
					...
					>
						<p>Add courses from the search bar. An entry will then appear in the workspace. You can click on the dropdown to select a section, and the class will appear on the calendar. You can enable and disable classes using the course toggle. To remove a class from your workspace, press the X button.</p>
						<p>
							In addition, this scheduler features an <em>automatic section selector</em> for your convenience. To use, simply unlock any number of courses in the workspace. This scheduler will then calculate all possible arrangements of sections for which none of the unlocked classes/sections will overlap. Use the left and right arrows to navigate these sections. The arrangements should automatically recalculate possible sections every time you enable/disable, lock/unlock, or add/remove a class. However, if a class is <em>locked</em>, we guarantee that the section number will not be changed.
						</p>
						<p>You can also limit sections be time. Above the calendar, you can change the allowed time range for any day of the week. The course scheduler should respect these times, and it will not generate arrangements with courses that start before the first time or end after the second. Note: If has a course doesn't have a time (marked as A), then the scheduler will leave it blank.</p>
						<p>
							We hope that this course schuduler makes your life easier! You can find the source code{" "}
							<a
