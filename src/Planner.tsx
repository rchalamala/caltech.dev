		...
		if (match !== null) {
			...
			for (const day of match[1]) {
				ret[day_to_i.indexOf(day)].push({
					start: new Date(2018, 0, day_to_i.indexOf(day) + 1, parseInt(match[2]), parseInt(match[3])),
					end: new Date(2018, 0, day_to_i.indexOf(day) + 1, parseInt(match[4]), parseInt(match[5])),
				});
			}
		}
	}
...
function Planner() {
	...
	const calEvents: DateData[] = CourseToDates(state.courses.filter((course) => course.enabled));
